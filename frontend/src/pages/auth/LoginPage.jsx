  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        const mockToken = "mock-jwt-token-motofam-2026";
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-md p-8 shadow-2xl space-y-6">
        ...
        {/* Role Selector Tabs */}
        <div className="grid grid-cols-3 gap-2 bg-surface-page p-1 rounded-xl border border-surface-border text-xs font-bold text-center">
          <button
            type="button"
            onClick={() => setRole("buyer")}
            className={`py-2 rounded-lg transition cursor-pointer ${role === "buyer" ? "bg-surface-primary text-white shadow-sm" : "text-surface-muted hover:text-surface-text"}`}
          >
            Buyer
          </button>
          <button
            ...
            className={`py-2 rounded-lg transition cursor-pointer ${role === "seller" ? "bg-surface-primary text-white shadow-sm" : "text-surface-muted hover:text-surface-text"}`}
          >
            Seller
          </button>
          <button
            ...
            className={`py-2 rounded-lg transition cursor-pointer ${role === "admin" ? "bg-surface-primary text-white shadow-sm" : "text-surface-muted hover:text-surface-text"}`}
          >
            Admin
          </button>
        </div>
        ...
        <div className="text-center pt-2 border-t border-surface-border text-xs text-surface-muted">
          ...
          <Link to="/register" className="text-surface-primary font-bold hover:underline">
            Register here
          </Link>
        </div>
      </div>
